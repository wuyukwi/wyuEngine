...
void Node3D::Start()
{
    ...
    BuildMatrixIdentity(m_ScalingMatrix);
...
void Node3D::Tick(float deltaTime)
{
    ...
    // スケーリング
    BuildMatrixScale(m_ScalingMatrix, m_scaling.x, m_scaling.y, m_scaling.z);

    // ワールドマトリックスの更新
    m_worldMatrix = m_ScalingMatrix * m_rotationMatrix * m_translationMatrix;

    //MatrixMultiBy(m_worldMatrix, m_ScalingMatrix, m_rotationMatrix);
    //MatrixMultiBy(m_worldMatrix, m_worldMatrix, m_translationMatrix);
...
void Node3D::SetScaling(Vector3f Scaling)
{
    m_scaling = Scaling;
}
...
{
    g_pEngine->GetRenderer()->LoadXModel(g_pEngine->GetPathManager()->GetFileFullPath(file_name), m_modelId);
}
...
void ModelNode::LoadModel(const std::string& file_name)
{
    g_pEngine->GetRenderer()->ReleaseXModel(m_modelId);
