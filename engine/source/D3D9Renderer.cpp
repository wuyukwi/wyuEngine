...
void D3D9Renderer::StartImGuiFrame(bool enable)
{
    ...
    {
        ImGui::Begin(u8"デバッグ", &show_gui_debugwindow,ImGuiWindowFlags_AlwaysAutoResize);

        const char* rendering_desc[] =
        {
            u8"ポリゴンを塗りつぶします",
            u8"ポリゴンを点で描画",
            u8"ポリゴンを線で描画します",
        };
        static int rendering_type = 0;
        ...
        ImGui::Combo(u8"ポリゴンの塗りつぶし方法", &rendering_type, rendering_desc, IM_ARRAYSIZE(rendering_desc));

        const char* cull_desc[] =
        {
            u8"ポリゴンのカリングを無効",
            u8"時計回りの頂点を持つポリゴンをカリング",
            u8"反時計回りの頂点を持つポリゴンをカリング",
        };

        static int cull_type = 0;
        ...
        ImGui::Combo(u8"ポリゴンのカリング", &cull_type, cull_desc, IM_ARRAYSIZE(rendering_desc));
        ...
        static bool MultisampleEnable = false;
        ImGui::Checkbox("MultisampleEnable", &MultisampleEnable);
        EnableMultiSample(MultisampleEnable);
