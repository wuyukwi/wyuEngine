 ...
 * @Author: コウ　キガク
 * @         黄   琦岳
 * @Date: 2022-06-01 11:02:03
 * @LastEditors: 黄琦岳
 * @LastEditTime: 2022-06-01 11:02:05
 * @FilePath: \wyuEngine\sources\Framework\Common\BMP.hpp
 * @Description:
                    ...
                    for (int32_t y = img.Height - 1; y >= 0; y--) {
                        for (uint32_t x = 0; x < img.Width; x++) {
                            (img.data + img.Width * (img.Height - y - 1) + x)->bgra =
                                *reinterpret_cast<R8G8B8A8Unorm*>(pSourceData + img.pitch * y +
                                    x * (img.bitcount >> 3));
                        }
                    }
                }
