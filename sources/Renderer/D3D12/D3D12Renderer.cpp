 ...
 * @LastEditors: 黄琦岳
 * @LastEditTime: 2022-05-27 11:07:07
 * @FilePath: \wyuEngine\RHI\d3d\D3D12Manager.cpp
 * @Description:
...
namespace wyuEngine {
    ...
    //template<class T>
    //inline void SAFE_RELEASE(T** ppInterfaceToRelease)
    //{
...
HRESULT D3D12Renderer::CreateDescriptorHeaps()
{
    HRESULT hr;
...
HRESULT D3D12Renderer::CreateRenderTarget()
{
    HRESULT hr;
...
HRESULT D3D12Renderer::CreateDepthStencil()
{
    HRESULT hr;
...
HRESULT D3D12Renderer::CreateVertexBuffer(const Buffer& buffer)
{
    HRESULT hr;
...
HRESULT D3D12Renderer::CreateIndexBuffer(const Buffer& buffer)
{
    HRESULT hr;
...
HRESULT D3D12Renderer::CreateTextureBuffer(const Image& image)
{
    HRESULT hr;
...
HRESULT D3D12Renderer::CreateSamplerBuffer()
{
...
HRESULT D3D12Renderer::CreateConstantBuffer(const Buffer& buffer)
{
...
HRESULT D3D12Renderer::CreateGraphicsResources()
{
    HRESULT hr;
...
void D3D12Renderer::Finalize()
{
    ...
    for (uint32_t i = 0; i < kFrameCount; i++) {
        SAFE_RELEASE(m_pRenderTargets[kFrameCount]);
    }
